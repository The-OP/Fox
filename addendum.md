...
Поддерживаемые на данный момент версии Tor Browser: **7**. <br>
...
Кроме того, `user.js` можно (и нужно) создавать в еще ни разу не запускавшемся профиле. Т.е.: распаковываем дистрибутив Tor Browser, идем папку профиля (`Browser/TorBrowser/Data/Browser/profile.default`) и записываем туда `user.js` с нужными настройками. Он применится при запуске и в результате у вас с самого начала будет настроенный профиль.
